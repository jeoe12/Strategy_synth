...
interface IExchangeRates {
    ...
    //function anyRateIsStale(bytes32[] memory currencyKeys) external view returns (bool);
...
contract strategyMA{
   ...
   string Name = 'ETHMA20';
   address owner;
   IExchangeRates public rates = IExchangeRates(0x29A74bBDFd3eBAE39BFF917AAF4dAE8D3d505cf0);//kovan
   ISynthetix public synth = ISynthetix(0x404469525f6Ab4023Ce829D8F627d424D3986675);//kovan
   ...
   function RatesETH (bytes32 currencyKey) public view returns(uint rate)  {
     uint ETHrate = rates.rateForCurrency(currencyKey);
     return ETHrate;
   }

   function RatesSNX (bytes32 currencyKey)  public view returns(uint rate)  {
     uint SNXrate = rates.rateForCurrency(currencyKey);
     return SNXrate;
   }

   function ETHMA20 (uint _ETHMA20) public view returns(uint)
   {
    require(msg.sender == owner, "Only the Owner can set the MA20Day");
    uint ETHMA = _ETHMA20; 
    return ETHMA;
   }


   function TradeStart(bytes32 sourceCurrencyKey , uint sourceAmount, bytes32 destinationKey, uint ETHMA) public returns (bool)
   {
      uint ETHrate = rates.rateForCurrency(sourceCurrencyKey);
     if 
     (ETHMA > ETHrate){
       // require(sourceCurrencyKey ==!0x7355534400000000000000000000000000000000000000000000000000000000);
        trade.SynthsUSD(sourceCurrencyKey, sourceAmount, destinationKey);
        return true;
        }
     else if (ETHMA < ETHrate) {
        //require(sourceCurrencyKey ==!'0x7345544800000000000000000000000000000000000000000000000000000000');
        trade.SynthsETH(sourceCurrencyKey, sourceAmount, destinationKey);
        return false;
     }   
     
   }

    function mintsUSD (bytes32 sourceCurrencyKey, uint sourceAmount, bytes32 destinationCurrencyKey) public returns(uint) {
     
    trade.sUSDtoSNX(sourceCurrencyKey, sourceAmount, destinationCurrencyKey); 
    synth.issueMaxSynths();  
         
    }

    function burnSNX () public returns (uint) {
    uint debt = synth.debtBalanceOf(msg.sender, "sUSD");
    // Burn for msg.sender = address(MyContract)
    synth.burnSynths(debt);
        
    }
    function ClaimReward (address addressValult) public returns (bool){
    synth.collateralisationRatio(addressValult);    
    IFeePool feePool = IFeePool(0xE24A751CCcD1bd5AE441F453d11520c6131b5633);
    feePool.claimFees();
